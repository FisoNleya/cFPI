...
import static persistance.Settings.*;
        ...
        List<String> processedReceiptData = new ArrayList<>();
            ...
            for(String line: rawReceiptData){
                ...
                try{
                    if (line.contains("@")){
                        line = line.trim();
                        String strQuantity = line.substring(line.indexOf(" "), line.indexOf("@")).trim();
                        qty = Integer.parseInt(strQuantity);
                        String strPrice = line.substring(line.lastIndexOf(" ")).trim();
                        price = new BigDecimal(strPrice);

                        String nextLine = rawReceiptData.get(rawReceiptData.indexOf(line) + 1);
                        description = nextLine.substring(0, nextLine.lastIndexOf(" ")).trim();
